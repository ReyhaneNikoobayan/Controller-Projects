...
Ti=0; Td=0; N=vars(4); 
if length(vars)==5
    k=vars(1); L=vars(2); T=vars(3); iC=vars(5); tt=0;
else
    Kc=vars(1); Tc=vars(2); kappa=(3); tt=1; 
end
if tt==0 % time based
    if typ == 1 % set point
           switch key
    ...
    elseif typ == 2 % disterbance rejection
        switch key
            case 2 % PI
           ...
           tt=L/T; 
           a1=PIDtab(1,ii+iC); b1=PIDtab(2,ii+iC); a2=PIDtab(3,ii+iC); b2=PIDtab(4,ii+iC); 
           Kp=a1/k*tt^b1; Ti=T/a2*tt^b2; 
           if key==3
               a3=PIDtab(5,ii+iC); b3=PIDtab(6,ii+iC); Td=a3*T*tt^b3; 
           end
    end
elseif tt == 1 % frequency based
    if typ == 1 % set point
        switch key
            case 2 % PI
                Kp=(4.264-0.148*kappa)/(12.119-0.432*kappa)*Kc;
                Ti=0.083*(1.935*kappa+1)*Tc;   
            case 3 % PID
                Kp=0.509*Kc; Td=0.125*Tc; Ti=0.051*(3.302*kappa+1)*Tc;
            case 4 % PI_D
                Kp=(4.437*kappa-1.587)/(8.024*kappa-1.435)*Kc;
                Ti=0.037*(5.89*kappa+1)*Tc;  Td=0.112*Tc;
        end
    elseif typ == 2 % disturbance rejection
        switch key
            case 2 % PI
                Kp=(1.892*kappa+0.244)/(3.249*kappa+2.097)*Kc;
                Ti=(0.706*kappa-0.227)/(0.7229*kappa+1.2736)*Tc;   
            case 3 % PID
                Kp=(4.434*kappa-0.966)/(5.12*kappa+1.734)*Kc;
                Td=0.144*Tc; Ti=(1.751*kappa-0.612)/(3.776*kappa+1.388)*Tc;
            case 4 % PI_D
                Kp=(4.437*kappa-1.587)/(8.024*kappa-1.435)*Kc;
                Ti=0.037*(5.89*kappa+1)*Tc;  Td=0.112*Tc;
        end
...
% writing the Gc transfer function
switch key
case 2 % PI
   nn=Kp*[Ti,1]; dd=[Ti,0];Gc_Sys=tf(nn,dd); H_Sys=1;
case 3 % PID
   nn=[Kp*Ti*Td*(N+1)/N, Kp*(Ti+Td/N), Kp];
   dd=Ti*[Td/N,1,0]; Gc_Sys=tf(nn,dd); H_Sys=1;
case 4 % PI_D
